...
func QueryAdminByFacultyID(facultiId uint) ([]*entity.User, error) {
	admin := make([]*entity.User, 0)
	result := database.DbInstance.Table("users").Where("faculty_id=?", facultiId).Find(admin)
